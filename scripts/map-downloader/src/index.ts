...
async function downloadAllMaps(area: string): Promise<void> {
  ...
  console.log(
    `境界: 北緯:${areaConfig.boundingBox.north}, 南緯:${areaConfig.boundingBox.south}`
  );
  console.log(
    `     東経:${areaConfig.boundingBox.east}, 西経:${areaConfig.boundingBox.west}\n`
  );
...
program
  .name("map-downloader")
  .description(
    "東京エリアの地図タイルダウンローダー (Tokyo Area Map Tile Downloader)"
  )
  .version("1.0.0");
...
program
  .command("quick-start")
  .description(
    "災害対策用の基本セットをダウンロード（東京23区、標準地図＋避難場所）"
  )
  .action(async () => {
...
program
  ...
  .action(() => {
    ...
    Object.entries(AREAS).forEach(([key, area]) => {
      console.log(`\n${key}: ${area.name}`);
      console.log(
        `  北緯: ${area.boundingBox.north}, 南緯: ${area.boundingBox.south}`
      );
      console.log(
        `  東経: ${area.boundingBox.east}, 西経: ${area.boundingBox.west}`
      );
    });
