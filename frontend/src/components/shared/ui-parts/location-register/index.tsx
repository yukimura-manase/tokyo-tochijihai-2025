...
export const LocationRegister = () => {
  const [registeredLocations, setRegisteredLocations] = useAtom(registeredLocationsAtom);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const newLocation: RegisteredLocation = {
        ...
        name: formData.name || (formData.type === LocationType.HOME ? '自宅' : '会社'),
        latitude,
        longitude,
      ...
      // ローカルストレージに保存
      localStorage.setItem('registeredLocations', JSON.stringify(updatedLocations));
        ...
        <div className="mt-6 space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <Button
                type="button"
                variant={formData.type === LocationType.HOME ? 'default' : 'outline'}
                onClick={() => setFormData({ ...formData, type: LocationType.HOME })}
                className="flex-1"
              >
                自宅
              </Button>
              <Button
                type="button"
                variant={formData.type === LocationType.OFFICE ? 'default' : 'outline'}
                onClick={() => setFormData({ ...formData, type: LocationType.OFFICE })}
                className="flex-1"
              >
                会社
              </Button>
          ...
          <div>
            ...
            <Input
              id="name"
              type="text"
              placeholder={formData.type === LocationType.HOME ? '自宅' : '会社'}
              value={formData.name}
